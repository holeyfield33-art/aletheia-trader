...
if page == "Dashboard":
        ...
        if signals:
            for sig in signals:
                with st.container(border=True):
                    ...
                    with col_left:
                        st.write(f"**{sig['agent_type'].upper()}** | {sig['instrument']} → `{sig['signal']}`")
                    ...
                    with col_right:
                        ...
                        entry_price = st.number_input("$", value=100.0, key=f"entry_{sig['signal_id']}")
                        
                        col_a, col_r = st.columns(2)
                        with col_a:
                            if st.button("✅ Approve", key=f"approve_{sig['signal_id']}", use_container_width=True):
                                try:
                                    resp = requests.post(
                                        f"{API_BASE}/v1/signals/approve",
                                        json={"signal_id": sig['signal_id'], "entry_price": entry_price, "qty": 1.0}
                                    )
                        ...
                        with col_r:
                            if st.button("❌ Reject", key=f"reject_{sig['signal_id']}", use_container_width=True):
                                try:
                                    requests.post(f"{API_BASE}/v1/signals/reject", json={"signal_id": sig['signal_id']})
                                    st.info("Rejected.")
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"Error: {e}")
        ...
        if orders:
            for order in orders:
                with st.container(border=True):
                    ...
                    with col_left:
                        st.write(f"**{order['order_id']}** | {order['instrument']} {order['side']} x{order['qty']}")
                        st.caption(f"Entry: ${order['entry_price']:.2f} | Opened: {order['executed_at'][:10]}")
                    
                    with col_right:
                        st.write("**Exit Price:**")
                        exit_price = st.number_input("$", value=order['entry_price'], key=f"exit_{order['order_id']}")
                        
                        if st.button("🔒 Close Position", key=f"close_{order['order_id']}", use_container_width=True):
                            try:
                                resp = requests.post(
        ...
        try:
            with st.spinner("Generating signal..."):
                ...
                if resp.status_code == 200:
                    ...
                    st.info(f"Signal expires in {data['expires_in_minutes']} minutes. Go to Dashboard to approve/reject.")
                else:
    ...
    try:
        ...
        if orders:
            ...
            # Display orders
            display_cols = ['order_id', 'instrument', 'side', 'qty', 'entry_price', 'exit_price', 'status', 'pnl']
    ...
    if st.button("Test API Connection"):
        try:
            ...
            st.info(f"Make sure the API server is running: `python api/server.py`")
